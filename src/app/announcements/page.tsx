...
const formatFirestoreTimestamp = (dateInput: string | { seconds: number; nanoseconds: number } | null | undefined): string => {
  if (!dateInput) {
    return 'N/A Date';
  }
...
export default function AnnouncementsPage() {
  ...
  const fetchAnnouncements = useCallback(async () => {
    ...
    setError(null);
    try {
      console.log(`[Frontend] Attempting to fetch announcements from: ${API_BASE_URL}/api/announcements`);
      const response = await fetch(`${API_BASE_URL}/api/announcements`);
      
      if (!response.ok) {
        const errorText = await response.text();
        console.error(`[Frontend] HTTP error fetching announcements! Status: ${response.status}, Text: ${errorText}`);
        throw new Error(`HTTP error! status: ${response.status} - ${errorText.substring(0, 100)}...`);
      }
      const data: Announcement[] = await response.json();
      console.log('[Frontend] Announcements fetched successfully:', data);
      console.log('[Frontend Announcements] Raw timestamp from fetched data (after JSON parse):', data[0]?.timestamp); 
      ...
      const commentsPromises = data.map(async (ann) => {
        ...
        try {
          console.log(`[Frontend] Attempting to fetch comments for ${ann.id} from: ${API_BASE_URL}/api/announcements/${ann.id}/comments`);
          const commentsResponse = await fetch(`${API_BASE_URL}/api/announcements/${ann.id}/comments`);
          if (!commentsResponse.ok) {
            const commentsErrorText = await commentsResponse.text();
            console.error(`[Frontend] Failed to fetch comments for ${ann.id}: Status: ${commentsResponse.status}, Text: ${commentsErrorText}`);
            return { annId: ann.id, comments: [] };
          }
          const commentsData: Comment[] = await commentsResponse.json();
          console.log(`[Frontend] Comments fetched for ${ann.id}:`, commentsData);
          return { annId: ann.id, comments: commentsData };
        } catch (commentErr: any) {
          console.error(`[Frontend] Error fetching comments for ${ann.id}:`, commentErr);
          return { annId: ann.id, comments: [] };
        }
      });
      ...
      setError(err.message || 'Failed to load announcements and comments. Please try again later.');
    } finally {
  ...
  const handleAddComment = async (announcementId: string) => {
    if (!user) {
      ...
      console.warn('[Frontend] Attempted to add comment without user logged in.');
      return;
    }
    ...
    try {
      ...
      console.log(`[Frontend] Posting comment for announcement ${announcementId} with text: "${text}"`);
      ...
      if (!response.ok) {
        ...
        console.error(`[Frontend] Failed to add comment: Status: ${response.status}, Error:`, errorData);
  ...
  const handleToggleReaction = async (announcementId: string, emoji: string) => {
    if (!user) {
      ...
      console.warn('[Frontend] Attempted to toggle reaction without user logged in.');
      return;
    }
    ...
    try {
      const idToken = await user.getIdToken();
      console.log(`[Frontend] Toggling reaction "${emoji}" for announcement ${announcementId}`);
      const response = await fetch(`${API_BASE_URL}/api/announcements/${announcementId}/reactions`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          'Authorization': `Bearer ${idToken}`,
      ...
      if (!response.ok) {
        const errorData = await response.json();
        console.error(`[Frontend] Failed to toggle reaction: Status: ${response.status}, Error:`, errorData);
        throw new Error(errorData.error || `Failed to toggle reaction: ${response.statusText}`);
      }

      const result = await response.json();
      const action = result.action;
      const currentCounts = result.currentCounts;
      console.log(`[Frontend] Reaction toggled. Action: ${action}, New Counts:`, currentCounts);
      setAnnouncements(prevAnnouncements =>
        prevAnnouncements.map(ann =>
          ann.id === announcementId ? { ...ann, reactionsCount: currentCounts } : ann
        )
      );
  ...
  const canManageAnnouncements = user && (userRole === 'admin' || userRole === 'mentor');
  ...
  const confirmDeleteAnnouncement = async () => {
    if (!announcementToDelete || !user) {
      setError('Error: No announcement selected for deletion or user not logged in.');
    ...
    try {
      ...
      const response = await fetch(`${API_BASE_URL}/api/announcements/${announcementToDelete}`, {
        method: 'DELETE',
        headers: {
          'Authorization': `Bearer ${idToken}`,
        },
      ...
      if (!response.ok) {
        const errorData = await response.json();
        console.error(`[Frontend] Failed to delete announcement: Status: ${response.status}, Error:`, errorData);
        throw new Error(errorData.error || `Failed to delete announcement: ${response.statusText}`);
      }

      console.log(`[Frontend] Announcement ${announcementToDelete} deleted successfully.`);
      setAnnouncements(prevAnnouncements =>
    ...
    <div className="min-h-screen bg-[var(--color-primary-dark)] text-[var(--color-primary-light)] flex flex-col">
      ...
      <main className="flex-grow container mx-auto p-6 md:p-12">
        ...
        {announcements.length === 0 && !pageLoading ? (
          <p className="text-gray-400 text-center text-lg">No announcements found. Check back later!</p>
        ) : (
          <div className="grid grid-cols-1 gap-6">
            {announcements.map((announcement) => (
              <div key={announcement.id} className="bg-gray-800 p-6 rounded-xl shadow-lg border border-gray-700 flex flex-col">
                <h2 className="text-2xl font-bold mb-2 text-[var(--color-accent-blue)]">{announcement.title}</h2>
                <p className="text-gray-400 text-sm mb-4">
                  By {announcement.authorName} on{' '}
                  {formatFirestoreTimestamp(announcement.timestamp)}
                </p>
                <p className="text-gray-300 leading-relaxed mb-4 flex-grow">{announcement.content}</p>
                <div className="flex items-center space-x-2 mb-4 pt-4 border-t border-gray-700">
                  {availableEmojis.map(emoji => {
                    const hasUserReacted = userReactions[announcement.id] === emoji;
                    const emojiCount = announcement.reactionsCount?.[emoji] || 0;
                    return (
                      <button
                        key={emoji}
                        onClick={() => handleToggleReaction(announcement.id, emoji)}
                      ...
                      >
                        <span>{emoji}</span>
                        {emojiCount > 0 && (
                          <span className="text-xs text-gray-300">{emojiCount}</span>
                        )}
                      </button>
                    );
                  })}
                ...
                <div className="mt-4 pt-4 border-t border-gray-700">
                  <h3 className="text-lg font-semibold mb-3 text-gray-200">Comments</h3>
                  <div className="space-y-3 mb-4 max-h-48 overflow-y-auto pr-2">
                    {(comments[announcement.id] || []).length === 0 ? (
                      ...
                      (comments[announcement.id] || []).map(comment => (
                        <div key={comment.id} className="bg-gray-700 p-3 rounded-lg">
                          <p className="text-gray-300 text-sm font-semibold">{comment.authorName}</p>
                    ...
                    <button
                      ...
                      disabled={!user || !newCommentText[announcement.id]?.trim()}
                    >
                      Post
                    </button>
                    </div>
                </div>
                {canManageAnnouncements && (userRole === 'admin' || userRole === 'mentor' || user?.uid === announcement.authorId) && (
