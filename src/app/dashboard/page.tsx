...
import { collection, query, where, orderBy, onSnapshot, doc, getDoc, getDocs, setDoc, updateDoc } from 'firebase/firestore';
interface Event {
  id: string;
  title: string;
  description: string;
...
export default function DashboardPage() {
  ...
  const [dailyInsight, setDailyInsight] = useState<{ title: string; content: string } | null>(null);
  ...
  useEffect(() => {
    fetch(`${process.env.NEXT_PUBLIC_BACKEND_URL}/api/announcements`)
      .then((response) => response.json())
      .then((data) => setRecentAnnouncements(data.map((announcement: any) => ({
          id: announcement.id,
  ...
  useEffect(() => {
    ...
    const unsubscribe = onSnapshot(q, async (snapshot) => {
      const fetchedConversations = await Promise.all(snapshot.docs.map(async (docSnapshot) => {
        ...
        const lastReadTimestamp = data.readBy && data.readBy[user.uid] ? data.readBy[user.uid]?.toDate() : null;
      ...
      const sortedConversations = fetchedConversations.sort((a, b) => {
        ...
        if (!a.isUnread && b.isUnread) return 1;
        const dateA = a.lastMessageAt && 'seconds' in a.lastMessageAt ? a.lastMessageAt.seconds : 0;
        const dateB = b.lastMessageAt && 'seconds' in b.lastMessageAt ? b.lastMessageAt.seconds : 0;
        return dateB - dateA;
      });
    ...
    <div className="min-h-screen bg-[var(--color-primary-dark)] text-[var(--color-primary-light)] flex flex-col">
      ...
      <main className="flex-grow container mx-auto p-6 md:p-12 flex flex-col items-center justify-center text-center">
        ...
        <p className="text-xl md:text-2xl text-gray-300 mb-8 max-w-2xl">
          Hello, {user.username}! You are logged in as a {userRole}.
          This is your personalized hub.
        </p>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 w-full max-w-5xl">
          <div className="lg:col-span-2 bg-gray-800 p-6 rounded-xl shadow-lg border border-gray-700 text-left flex flex-col">
            <h2 className="text-2xl font-bold mb-4 text-[var(--color-accent-yellow)]">Daily Insight 💡</h2>
            {isDailyInsightLoading ? (
              <p className="text-gray-400 text-lg">Loading today's insight...</p>
            ) : (
              dailyInsight && (
                <p className="text-gray-300 text-lg mb-4">
                  <span className="font-bold">{dailyInsight.title}</span> {dailyInsight.content}
                </p>
              )
            )}
          </div>
          <div className="lg:col-span-2 bg-gray-800 p-6 rounded-xl shadow-lg border border-gray-700 text-left flex flex-col">
            <h2 className="text-2xl font-bold mb-4 text-[var(--color-accent-blue)]">Recent Announcements</h2>
            <ul className="space-y-3 flex-grow">
              {recentAnnouncements.map((announcement: Announcement) => (
                <li key={announcement.id} className="border-b border-gray-700 pb-3 last:border-b-0">
                  <h3 className="text-lg font-semibold text-gray-200">{announcement.title}</h3>
                  <p className="text-gray-400 text-sm">{announcement.content}</p>
                  <p className="text-gray-500 text-xs mt-1">{formatTimestamp(announcement.timestamp)}</p>
                </li>
              ))}
            </ul>
            <div className="text-right mt-auto">
              <Link href="/announcements" className="text-blue-500 hover:text-blue-700 text-sm font-bold">View All Announcements &rarr;</Link>
            </div>
          </div>
          <div className="lg:col-span-2 bg-gray-800 p-6 rounded-xl shadow-lg border border-gray-700 text-left flex flex-col">
            <h2 className="text-2xl font-bold mb-4 text-[var(--color-accent-blue)]">Recent Messages</h2>
            {loadingConversations ? (
              <p className="text-center text-gray-400">Loading messages...</p>
            ) : recentConversations.length > 0 ? (
              <ul className="space-y-3 flex-grow">
                {recentConversations.map((conv) => (
                  <li key={conv.id} className={`p-3 rounded-lg flex items-center space-x-3 transition duration-150 border-b border-gray-700 last:border-b-0`}>
                    {conv.isUnread && (
                      <span className="w-2.5 h-2.5 bg-red-500 rounded-full flex-shrink-0 animate-pulse"></span>
                    )}
                    <div className="flex-grow">
                      <h3 className="text-lg font-semibold text-gray-200">{conv.otherParticipantName}</h3>
                      <p className="text-sm text-gray-400 truncate">{conv.lastMessageContent}</p>
                      <p className="text-xs text-gray-500 mt-1">{formatTimestamp(conv.lastMessageAt)}</p>
                    </div>
                  </li>
                ))}
              </ul>
            ...
            <div className="text-right mt-auto">
              <Link href="/messages" className="text-blue-500 hover:text-blue-700 text-sm font-bold">View All Messages &rarr;</Link>
            </div>
          </div>
          <div className="lg:col-span-2 bg-gray-800 p-6 rounded-xl shadow-lg border border-gray-700 text-left flex flex-col">
            ...
            <div className="text-right mt-auto">
              <Link href="/events" className="text-blue-500 hover:text-blue-700 text-sm font-bold">View Full Calendar &rarr;</Link>
            </div>
          </div>
        </div>
      </main>
